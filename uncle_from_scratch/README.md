# UnCLe from scratch: start with the data

This is a separate, incremental implementation alongside `../uncle`. The first
stage loads and explores Tiny ImageNet. Model training, hypernetworks, and
unlearning will build on this stage later.

Start with `01_tiny_imagenet.ipynb` and run the cells in order. It walks through
...
```powershell
python -m pip install -r hypernetwork/uncle_from_scratch/requirements.txt
python -m jupyterlab hypernetwork/uncle_from_scratch/01_tiny_imagenet.ipynb
```

For the same exploration as a script:

```powershell
python hypernetwork/uncle_from_scratch/explore.py
```

It reuses `hypernetwork/data/tiny-imagenet-200` and saves example images, a class
...
`data.py` reads validation labels from `val_annotations.txt`, uses one sorted
...
`explore.py` uses a fixed seed and estimates RGB statistics from 2,048 training
...
Continue with `02_task1.ipynb`, or prepare the task from the repository root:

```powershell
python hypernetwork/uncle_from_scratch/task1.py
```

Task IDs are **zero-based**: task ID `1` is the second group, following task `0`.
`tasks.py` sorts the 200 WordNet IDs, shuffles them using NumPy PCG64 with seed
42, and divides them into 20 disjoint groups of ten. The full assignment is
saved in `task_partition.json` and checked on subsequent runs. A conflicting
...
`task1.py` saves the measured class mapping, counts, batch shapes, and an example
...
```powershell
python hypernetwork/uncle_from_scratch/test_tasks.py
```
...
- `streams.py` turns the saved partition into the `{task: {"train", "test"}}`
...
- `baseline.py` trains the target network on one task with ordinary backprop.
...
- `run.py` works through a request sequence and reports the four numbers.
...
!pip install -q -r repo/hypernetwork/uncle_from_scratch/requirements.txt

import sys
sys.path.insert(0, "/content/repo/hypernetwork/uncle_from_scratch")
...
```powershell
python hypernetwork/uncle_from_scratch/baseline.py --task 1 --epochs 5
python hypernetwork/uncle_from_scratch/run.py --backbone cnn --chunks 32 --epochs 1 --limit-requests 5 --max-images 300
python hypernetwork/uncle_from_scratch/run.py --sequence 1 --backbone resnet50
```
